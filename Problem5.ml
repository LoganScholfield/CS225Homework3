...
(* 
   ...
   NOTE : This function is incomplete for ARITH as defined in the template.
*)
let rec redx e = match e with
  ...
  | And(Bool(false), e) -> Bool(false)
  | Or(Bool(true), e) -> Bool(true)
  ...
  (* | Plus(e1,e2) -> if isval e1 then Plus(Nat(e1)) *)
